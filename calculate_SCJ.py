...
def calculate_SCJ(tree, reconstructedAdj, extantAdjacencies_species_adj):
    ...
    for node in tree.iter_descendants("postorder"):
        ...
        elif node.name+".Chr" in extantAdjacencies_species_adj:
            set_of_node_adj=set(extantAdjacencies_species_adj[node.name+".Chr"])
        ...
        for adj in set_of_node_adj:
           ...
           # print str(adj)+'__'+str(mirrored_adj)
            if adj in set_of_nodes_parent_adj or mirrored_adj in set_of_nodes_parent_adj:
                scj_node -= 1  #if the current adj is also at the node's parent (even if it's mirrored), deduct 1 from scj_node
